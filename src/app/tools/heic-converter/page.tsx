...
import { Images, Download, Loader2, FileImage } from "lucide-react"
...
export default function HeicConverterPage() {
  ...
  const handleConvert = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        const blob = await convertHeic(files[i], targetFormat)
  ...
  const handleDownloadAll = async () => {
    ...
    a.href = url; a.download = "heic_converted.zip"; a.click()
    URL.revokeObjectURL(url)
  }
        ...
        {!results && (
          <>
            <FileUploader
              ...
              hint="Convert iPhone HEIC photos to JPG or PNG. Select multiple files at once."
            />
                ...
                <motion.div className="space-y-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  ...
                  <div className="flex justify-center">
                    <Button size="lg" className="gradient-brand text-white shadow-xl shadow-brand-500/25 h-12 px-10 rounded-xl text-base" onClick={handleConvert} disabled={processing}>
                      {processing ? <><Loader2 className="size-5 animate-spin mr-2" />{status}</> : <><Images className="size-5 mr-2" />Convert {files.length} File(s)</>}
                    </Button>
                  </div>
